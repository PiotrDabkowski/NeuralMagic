                ...
                with tf.variable_scope(None, default_name='DenseUnit'):
                    if self.bottleneck:
                        with tf.variable_scope('bottleneck'):
                            ...
                            if cand!=bottleneck_channels:
                                print 'Bottleneck too thin! Input channels %d. Changed from proposed %d to %d' % (input_channels, cand, bottleneck_channels)
                            if bottleneck_channels < self.bottleneck*input_channels*2:
                                ...
                                print 'Actually, there is no point in bottleneck at all in this case :)'
                    # BN -> RELU -> CONV
                    inp = tf_layers.batch_norm(inp, **self.batch_norm_params)
                    inp = self.activation(inp)
    ...
    def __call__(self, inp):
        ...
        with tf.variable_scope(None, default_name='DenseTransitionLayer'):
            if self.keep_dim_fraction != 1:
                ...
                reduced = inp
            inp = tf_layers.avg_pool2d(reduced, 2, 2)
        print inp.get_shape()
        return inp, reduced
